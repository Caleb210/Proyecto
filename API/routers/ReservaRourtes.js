...
router.post('/', async (req, res) => {
        try {
            const datosReserva = new Reserva(req.body);
            await datosReserva.save();
            res.status(201).json(datosReserva);
...
router.get('/', async(req, res) =>{
        const listaDatos = await Reserva.find();
        res.json(listaDatos);
...
router.get('/:id', async(req, res) =>{
        const listaDatos = await Reserva.findOne({id: req.params.id});
        if (listaDatos) {
            res.json(listaDatos);
        }
        else{
...
router.put('/:id', async(req, res) =>{
        const dato = await Reserva.findOneAndUpdate({
...
router.delete('/:id', async(req, res) =>{
        const dato = await Reserva.findOneAndDelete({id: req.params.id});
        if (dato) {
